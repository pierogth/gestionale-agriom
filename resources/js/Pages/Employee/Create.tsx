...
const App = () => {
 ...
 return (
   <div style={{backgroundColor:"#111827"}} className="dark:bg-grey-900 min-h-screen py-6 flex flex-col justify-center sm:py-12">
      <div className="dark:bg-gray-900 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          ...
          {errors.credit !== null ? <small style={{color:"red"}}>{errors.credit}</small>: ''}
             ...
             {errors.workhours !== null ? <small style={{color:"red"}}>{errors.workhours}</small>: ''}
              ...
              <input
                type="number"
                name="ehours"
                value={formData.ehours}
                onChange={handleInputChange}
                className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
             />
             {errors.ehours !== null ? <small style={{color:"red"}}>{errors.ehours}</small>: ''}
                     </div>
